...
\begin{problem} \textit{Good to practice linear algebra skills.} Let's prove Theorem 1. 
  ...
  \begin{enumerate}[label=\alph*.]
   ...
   \begin{tikzpicture}
    ...
    \item Show that $p_t(x)$ is a sum of solutions of this form. \textcolor{red}{(give more hints)}
...
\begin{enumerate}[label =\alph*.]
 ...
 $$\mathbf{E}(WB - \mathbf{E}(W)\mathbf{E}(B))$$
 ...
 $$WB - \mathbf{E}(W)\mathbf{E}(B) \ : \ \Omega \ \to \ \mathbf{R}$$
...
\begin{tikzpicture}
    ...
    \node[above, red] at ({0.866*1},{1 - 0.5*1 + 0.3}) {$x$};
    \node[above, red] at ({0.866*2},{1 - 0.5*2 + 0.3}) {$y$};
 \end{tikzpicture}
 \end{center} 
 \begin{enumerate}[label =\alph*.]
...
\begin{enumerate}[label = \alph*.]
 \item Show that for every domino tiling, $\Phi(k)$ is a constant, i.e. independent of $k$.
...
\begin{problem}
  Show that any tiling of the hexagonal $A\times B\times C$ graph by lozenges can be obtained from any other by a sequence if \textit{flips}
\begin{center}
\begin{tikzpicture}
 ...
 \end{tikzpicture}
 \end{center} 
 What about the $m\times n$ rectangle? What about any square graph (subgraph of $\mathbf{Z}^2$)?

 \begin{enumerate}[label = \alph*.]
  \item What does the hexagonal flip $T \leftrightarrow T'$ do to the height function? i.e. how is $h_{T'}$ related to $h_T$?
  \item Define a height function for the $m\times n$ rectangle which has an analogous property to a. 
