...
\setcounter{problem}{0}
\noindent
\textit{Added post-Mattekollo: If you enjoyed this course, try reading ``Random walks on electrical networks'' by Doyle and Snell \url{https://math.dartmouth.edu/~doyle/docs/walks/walks.pdf}, or (more advanced) ``Probability on trees and networks'' by Lyons and Peres \url{https://rdlyons.pages.iu.edu/prbtree/book_pb.pdf}, or just email me for more suggestions.}
...
\end{itemize}
Both parts are true by problem 4 of the 6 July problem set - convince yourself that 4 indeed implies these.
...
\begin{theorem}
    Let $G$ be a connected graph where there is exactly zero or one edge between each vertex, and $x,y\in V(G)$ be two vertices. Then
...
\begin{problem}
  ...
  \begin{enumerate}[label =\alph*.]
    \item Show that the resistance of a graph decreases or stays the same when we add edges.
   \item Show that $R(0,B_n)$ is at least the effective resistance $R_n$ between $0$ and $n$ in the graph 
   \begin{center}
   \begin{tikzpicture}
    ...
    \end{center}
    where there are $12i$ edges between vertex $i$ and $i+1$ for $i\ge 1$, and $4$ edges between $0$ and $1$.
    ...
    \item Conclude that 
    $$\lim_{n \to \infty}R_n \ = \ \frac{1}{4} + \frac{1}{12} + \frac{1}{24}+ \cdots \ \ = \ \frac{1}{4}+\sum_{i \ge 1} \frac{1}{12i} \ = \ \infty.$$
    \item \textbf{The random walker comes back!} Conclude that 
    $$1-P \ = \ \lim_{n \to \infty}P(0,B_n) \ = \ \lim_{n \to \infty} \frac{1}{R(0,B_n)} \ \le \ \lim_{n \to \infty} \frac{1}{R_n} \ = \ 0$$
...
\begin{enumerate}[label =\alph*.]
 ...
 \item Show that this is the \textit{unique} water network of the form
  $$W \ = \ (G,1,?,?,I_x)$$
  for the function $I_x$ you defined. \textit{Hint:} use problem 4 of the 6 July problem set.

  Thus - we have an interpretation of $\mathbf{P}_x(\tau_A < \tau_B)$ as the pressure at $x$ in a water network.
 \item \textbf{Extra problem}. Show that if $a,b\in V(G)$, 
