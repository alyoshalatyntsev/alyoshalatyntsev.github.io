...
\begin{problem}
   We open up the tubes at $x$ and $y$, and add small wooden sheets to keep in the water. We will compute the resistance between $x$ and $y$. 
    \begin{enumerate}[label =\alph*.]
     \item s
...
\begin{problem}
  ...
  $$R(x,y) \ = \ \mathbf{E}[\# \textup{steps a random walk }\gamma \textup{ starting at } x\textup{ takes before first hitting }y].$$
  \begin{enumerate}[label =\alph*.]
   \item 
...
\begin{problem}
  ...
  \begin{enumerate}[label =\alph*.]
   ...
   \item Show that joining two graphs $G_1$ and $G_2$ together along a single vertex $G_1\cdot_y G_2$
\begin{center}
\begin{tikzpicture}
% Draw 8 vertices at arbitrary positions
\foreach \i/\x/\y in {1/0/0, 2/2/0.5, 3/1/2, 4/3/2, 5/4/0.5, 6/5/1.5, 7/3.5/3, 8/1.5/3.5} {
...
\node[above] at (v15) {$z$};
 ...
 gives $R_{G_1\cdot G_2}(x,z)= R_{G_1}(x,y)+R_{G_2}(y,z)$.
   ...
   \item Show that adding two graphs in parallel $G_1+G_2$
   \begin{center}
   \begin{tikzpicture}
    % Draw two blue dots
...
\begin{problem}
  ...
  \begin{tikzpicture}
    ...
    % Mark x and y
    \node[above left, font=\bfseries] at (v1) {$x$};
    \node[below right, font=\bfseries] at (v6) {$y$};
   \end{tikzpicture}
   \end{center} 
\end{problem}
...
\begin{problem}
   ...
   \begin{tikzpicture}
    ...
    % Add labels 1,2,3,4,5 above the vertices
    \foreach \i in {1,...,5} {
        \node[above] at (v\i) {\i};
    }
    \end{tikzpicture}
