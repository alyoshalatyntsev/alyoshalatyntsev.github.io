...
\begin{axiom}\textbf{(Addition of resistances)} Putting two tubes $T_1$ and $T_2$ after one another adds their resistances:
\begin{center}
\begin{tikzpicture}
...
We represent a collection of tubes joined together as a graph 
\begin{center}
\begin{tikzpicture}
% Draw 8 vertices at arbitrary positions
\foreach \i/\x/\y in {1/0/0, 2/2/0.5, 3/1/2, 4/3/2, 5/4/0.5, 6/5/1.5, 7/3.5/3, 8/1.5/3.5} {
...
\node[above] at (0.75,0.2) {$R$};
 ...
 \end{tikzpicture}
 \end{center} 
where the ends of the graph are considered closed off.

\begin{problem}
   We open up the tubes at $x$ and $y$, and add small wooden sheets to keep in the water. 
   
   In the following problem, where you may make reasonable assumptions about flow of water, etc., we will compute the resistance between $x$ and $y$. 
    \begin{enumerate}[label =\alph*.]
...
\node[above] at (v15) {$z$};
 ...
 gives $R_{G_1\cdot G_2}(x,z)= R_{G_1}(x,y)+R_{G_2}(y,z)$.
   \item What happens if we join two graphs $G_1$ and $G_2$ together along multiple points $y_1, \ldots, y_k$? 
   ...
   gives $\frac{1}{R_{G_1+G_2}(x,y)} = \frac{1}{R_{G_1}(x,y)} + \frac{1}{R_{G_2}(x,y)}$.
     \item What is the resistance of the above graph between $x$ and $y$?
    \end{enumerate}
...
\begin{definition}
    A \emph{random walk} on a graph $G$ starts at a given vertex $x$ and at each step moves to a neighbouring vertex chosen uniformly at random. 
\end{definition}
...
\begin{problem}
  ...
  \begin{enumerate}[label =\alph*.]
   \item The \textit{expected hitting time} $H(x,y)$ between two vertices $x$ and $y$ is the expected number of steps a random walk starting at $x$ takes before first hitting $y$. Show that the \textit{commute time} $\kappa(x,y)=H(x,y)+H(y,x)$ is the expected number of steps a random walk starting at $x$ takes to hit $y$, and then return to $x$.
   \item Show that 
       $$H(x,y) \ = \ 1 \ + \ \frac{1}{\deg(x)}\sum_{z\sim x} H(z,y)$$
       and conclude $H(x,y)= p_y-p_x$.
   \item Show that 
    $$R(x,y) \ = \ \frac{1}{2|E(G)|} \kappa(x,y) $$
    where $|E(G)|$ is the number of edges in the graph $G$.
    
    
    \textit{Hint}. Externally add $\deg(z)$ units of water per second to each vertex $z$, and remove $\sum \deg(x) = 2|E(G)|$ units of water per second from $y$. Show that 
       $$p_x-p_y \ = \ 1 \ + \ \frac{1}{\deg(x)}\sum_{z\sim x} (p_z-p_y).$$
    Then externally add $\deg(z)$ units of water per second to each vertex $z$, and remove $\sum \deg(x) = 2|E(G)|$ units of water per second from $y$. 
