...
\begin{itemize}
 ...
 \item The value $R_G(x,y)$ should not depend on the choice of $W$.
\end{itemize}
...
\foreach \x in {0,...,6} {
  ...
  \end{center}
  \begin{enumerate}[label =\alph*.]
   \item Compute the number of edges between the boundaries of the $i\times i$ and $(i+1)\times (i+1)$ diamonds. 
    \item What is the expected time a random walk starting at the origin takes to hit the edge, then return to the origin? \textit{Hint:} use question 4d, and reduce this to a problem about a simpler graph, which you should compute the resistance of.
    \item Conjecture what the value of $\kappa(0,\textup{edge})$ would approximately look like for diamonds in dimension other than two.
 \end{enumerate}
\end{problem}
...
\begin{problem} \textit{Gambler's ruin.} Let $G$ be a graph and $A,B \subseteq G$ subsets. 
\begin{enumerate}[label =\alph*.]
...
\end{enumerate}
Thus - we have an interpretation of $\mathbf{P}_x(\tau_A < \tau_B)$ as the pressure at $x$ in a water network. How do you interpret the flows $I_e,I_x$? Write down what you expect the result to be if we remove the condition that all $R_e=1$.
\end{problem}
